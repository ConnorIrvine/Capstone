...
def get_user_input():
    ...
    try:
        ...
        if duration < 45:
            print("Duration must be at least 45 seconds for HRV calculation!")
...
def simulate_realtime_analysis(input_file, duration, sim_speed=10.0, sampling_rate=100):
    ...
    FIRST_CALCULATION_TIME = 45  # First calculation at 45s (using data from 15s-45s)
    ...
    try:
        ...
        print(f"HRV calculated at 45s (15s-45s window), then every 30s")
        print(f"{'='*60}")
        print("\nStatus Colors:")
        ...
        print(f"{'='*60}")
        print("\nCollecting initial data (45 seconds)...\n")
        ...
        with open(input_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                # Only start collecting data after 15 seconds
                if sim_elapsed >= 15:
                ...
                # Perform HRV calculation at 45s, then every 30s after that
                should_calculate = False
                if window_count == 0 and sim_elapsed >= FIRST_CALCULATION_TIME:
                    # First calculation at 45s
                    should_calculate = True
                    last_calculation_time = sim_elapsed
                elif window_count > 0 and sim_elapsed - last_calculation_time >= UPDATE_INTERVAL_SEC:
                    # Subsequent calculations every 30s
                    should_calculate = True
                    last_calculation_time = sim_elapsed
                
                if should_calculate and len(ppg_buffer) >= window_size_samples * 0.8:  # Allow 20% tolerance
                    window_count += 1
