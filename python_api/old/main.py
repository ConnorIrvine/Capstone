...
def analyze_window_quality(ppg_window: np.ndarray, sampling_rate: float, 
                           segment_sec: int = 3, max_bad_segments: int = 0) -> tuple:
    """
...
async def analyze_hrv(request: HRVRequest):
    ...
    if len(ppg_array) < min_samples:
        raise HTTPException(
            ...
            detail={
                "success": False,
                "error": f"Insufficient data: need at least {min_samples} samples ({10} seconds), got {len(ppg_array)}",
                "bad_segments": 0
            }
        )
    
    # Perform signal quality check
    ...
    if is_bad:
        raise HTTPException(
            ...
            detail={
                "success": False,
                "error": f"Poor signal quality: {bad_segments} bad segments detected (max allowed: {request.max_bad_segments})",
